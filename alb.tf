...
resource "aws_lb_listener" "fiap_pedidos" {
  load_balancer_arn = aws_lb.fiap_pedidos.arn
  port              = 80
  protocol          = "HTTP"
  default_action {
    type             = "forward"
  ...
  depends_on = [aws_lb_target_group.fiap_pedidos]
}

resource "aws_lb_target_group" "fiap_pedidos" {
